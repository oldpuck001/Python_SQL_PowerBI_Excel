...
def export_tree_new_xlsx(target_file_path,
                         in_groupby_2_summary_df, in_groupby_2_summary_df_label, in_priority_list, in_groupby_3_summary_dict,
                         in_summary_total, out_summary_total,
                         out_groupby_2_summary_df, out_groupby_2_summary_df_label, out_priority_list, out_groupby_3_summary_dict
                         ):
    ...
    # 添加In数据到左边
    in_cycle_not_sorted = list(in_groupby_2_summary_df[in_groupby_2_summary_df_label])
    ...
    for in_groupby in in_cycle:
        ...
        in_groupby_value = in_groupby_2_summary_df.query('in_groupby_2_sort == @in_groupby')['in_groupby_2_value'].values[0]
        col_d = ws.cell(row=in_step, column=4, value=in_groupby_value)
        cell_format_fun.cell_format(col_d, 2)

        in_skip = len(in_groupby_3_summary_dict[in_groupby]) + 1

        secondary_in_step = in_step
        for row in dataframe_to_rows(in_groupby_3_summary_dict[in_groupby], index=False, header=False):
    ...
    # 添加Out数据到右边
    out_cycle_not_sorted = list(out_groupby_2_summary_df[out_groupby_2_summary_df_label])
    ...
    for out_groupby in out_cycle:
        ...
        out_groupby_value = out_groupby_2_summary_df.query('out_groupby_2_sort == @out_groupby')['out_groupby_2_value'].values[0]
        col_h = ws.cell(row=out_step, column=8, value=out_groupby_value)
        cell_format_fun.cell_format(col_h, 2)

        out_skip = len(out_groupby_3_summary_dict[out_groupby]) + 1
        
        secondary_out_step = out_step
        for row in dataframe_to_rows(out_groupby_3_summary_dict[out_groupby], index=False, header=False):
